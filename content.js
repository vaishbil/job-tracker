...
function scrapeJob() {
    ...
    if (site === "indeed") {
      ...
      if (!company) {
        const companyEl = document.querySelector(
          '[data-company-name], [class*="companyName"], a[data-tn-element="companyName"]',
        );
    ...
    if (site === "naukri") {
      ...
      if (!title) {
        const h1 = document.querySelector(
          'h1.styles_jd-header-title__rZwM1, h1[class*="title"], h1',
        );
      ...
      if (!company) {
        const companyEl = document.querySelector(
          'a.styles_comp-name__PXZZo, [class*="comp-name"], a[class*="company"]',
        );
        if (companyEl) company = companyEl.innerText.trim();
      }
    }
  ...
  if (!title || !company) {
    ...
    if (site === "indeed" && t.includes(" - ")) {
      ...
      if (
        !title &&
        !possibleTitle.toLowerCase().includes("jobs in") &&
        !possibleTitle.toLowerCase().includes("welcome") &&
        !possibleTitle.toLowerCase().startsWith("welcome,")
      ) {
        title = possibleTitle;
      }
